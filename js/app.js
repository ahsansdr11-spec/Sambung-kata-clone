...
(function () {
  ...
  // Safe click-binder. NOTE: `el?.onclick = fn` is INVALID JS (SyntaxError:
  // "Invalid left-hand side in assignment") because the assignment target
  // can't contain optional chaining. That bug previously crashed this whole
  // file on load (breaking login, search, everything). Use this helper instead.
  function on(sel, fn){ const el = typeof sel === 'string' ? $(sel) : sel; if (el) el.onclick = fn; return el; }
  ...
  function tierClass(t){
    if (t>=4) return 'tier-dead';
    if (t===3) return 'tier-hard';
    if (t===2) return 'tier-mid';
    return 'tier-easy';
  }

  function initMagic(){
    const bar=$('#magicBar'); if(!bar) return;
    let t=DataLayer.getTactical().slice(0,16); if(!t.length) t=['oa','ez','ks','tt','x','z','q'].map(s=>({s,t:1}));
    bar.innerHTML='';
    t.forEach(x=>{ const b=document.createElement('button'); b.className='magic-chip '+tierClass(x.t||1); b.textContent=x.s.toUpperCase(); b.onclick=()=>{ $('#prefixInput').value=x.s; Search.run(); }; bar.appendChild(b); });
  }
  ...
  function initControls(){
    ...
    on('#clearBtn', ()=>{if(p)p.value='';Search.run();});

    on('#modeToggle', ()=>{State.mode=State.mode==='fast'?'normal':'fast';syncUI();Search.run();});
    on('#statusToggle', ()=>{State.status=State.status==='all'?'verified':State.status==='verified'?'unverified':'all';save();syncUI();Search.run();});
    on('#trapToggle', ()=>{State.trap=!State.trap;syncUI();Search.run();});
    on('#hideToggle', ()=>{State.hide=!State.hide;syncUI();Search.run();});
    on('#brutalToggle', ()=>{State.brutal=!State.brutal;syncUI();Search.run();});
    on('#fullToggle', ()=>{State.fullWidth=!State.fullWidth;syncUI();Search.run();});
    on('#sortToggle', ()=>{State.sort=State.sort==='none'?'asc':'none';save();syncUI();Search.run();});
    on('#randomBtn', ()=>{const w=DataLayer.getRandomWord();if(w&&p){p.value=w;Search.run();}});
    on('#showHiddenBtn', ()=>{});
  }
